...
function iterate(real, imag, x, y) {
  var n = 0;
...
function draw() {
  ...
  for (var row = 0; row < width; row++) {
    for (var col = 0; col < height; col++) {
      ...
      var n = iterate(z0re, z0im, cre, cim);
      if (n == iter) {
        stroke(0);
      } else{
        a = pal[n%20];
  ...
  noStroke();
  fill(255);
  text("z_0 = " + str(z0re) + " + " + str(z0im) + "i", width/22, height/15);
...
function mousePressed() {
  z0re = rmin + (mouseX / width) * (rmax - rmin);
  z0im = imax - (mouseY / height) * (imax - imin);
